...
g <- ggplot() +
  ...
  theme(axis.ticks.length = unit(-3, "mm"), # 目盛り線の長さ
        axis.ticks = element_line(linewidth = 1), # 目盛り線の太さ
        ...
        panel.background = element_rect(fill = "white", colour = "black", linewidth = 1.2), # fillは枠内の色、colourは枠線の色、sizeは枠線の太さ
...
# 指数回帰描画 ####
g <- g +
  geom_smooth(data = df1, aes(x = t, y = mean), method = "nls",
              formula = y ~ a * exp(b * x), se = FALSE,
              method.args = list(start = list(a = 3000, b = 0.02)),
              linewidth = 1, colour = "blue")
  #stat_smooth(data = df1, aes(x = t, y = mean), method = "nls",
  #            formula = "y ~ a * exp(b * x)", se = FALSE,
  #            method.args = list(start = list(a = 3000, b = 0.02)),
  #            linewidth = 1, colour = "blue")

############## Rのバージョンによっては上の指数回帰描画のスクリプトだと ################
############## 対数軸で直線に描画されない場合があるのでそのときは下を使う #############
## 指数回帰
#x <- df1$t
#y <- df1$mean
...
#df1 <- cbind(df1, y_est)
## 指数回帰描画
#g <- g + geom_line(data = df1, aes(x = t, y = y_est), linewidth = 1, colour = "blue")
#######################################################################################
